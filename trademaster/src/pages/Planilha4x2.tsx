...
const STORAGE_KEY = 'guias_planilha_4x2';
...
function carregarDados(): Dados4x2 {
  try {
...
export default function Planilha4x2({ onVoltar }: Props) {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  const [dados, setDados] = React.useState<Dados4x2>(carregarDados);
  ...
  const op2a = maoFixa + lucroOp1a;
  const lucroOp2a = op2a * (payoutPct / 100);
  const lucroTentativa1 = lucroOp1a + lucroOp2a;
  // Tentativa 2 (mesma lógica, mesmos valores)
  const op1b = maoFixa;
  const lucroOp1b = maoFixa * (payoutPct / 100);
  const op2b = maoFixa + lucroOp1b;
  const lucroOp2b = op2b * (payoutPct / 100);
  const lucroTentativa2 = lucroOp1b + lucroOp2b;

  // Controle mensal
  const bancaAcumulada = dados.dias.map(() => ({ bancaVal: 0, pct: 0 }));
            ...
            <div className="px-6 pb-6 space-y-5">
              ...
              <div className="flex flex-wrap items-end gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className={cn('px-4 py-2 rounded-lg text-sm font-black text-center', lucroTentativa1 >= 0 ? 'bg-green-600 text-white' : 'bg-red-600 text-white')}>
                    {lucroTentativa1.toFixed(2).replace('.', ',')}
                  </div>
                </div>
              </div>

              {/* Tentativa 2 — Soros auto-calculado */}
              <div className="flex flex-wrap items-end gap-4">
                <div className={cn('px-3 py-1.5 rounded-lg text-xs font-black uppercase', 'bg-orange-500 text-black')}>
                  TENTATIVA 2
                </div>
                {[
                  { label: 'OPERAÇÃO 1B', value: op1b },
                  { label: 'OPERAÇÃO 2B', value: op2b },
                ].map(item => (
                  <div key={item.label} className="space-y-1">
                    <span className="text-[10px] font-bold text-slate-400 uppercase">{item.label}</span>
                    <div className={cn('w-28 px-3 py-2 rounded-lg text-sm font-bold text-center', isDark ? 'bg-slate-800 text-white border border-slate-600' : 'bg-gray-800 text-white border border-gray-600')}>
                ...
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">LUCRO</span>
                  <div className={cn('px-4 py-2 rounded-lg text-sm font-black text-center', lucroTentativa2 >= 0 ? 'bg-green-600 text-white' : 'bg-red-600 text-white')}>
                    {lucroTentativa2.toFixed(2).replace('.', ',')}
                  </div>
                </div>
              </div>
            </div>
